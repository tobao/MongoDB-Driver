...
async function main() {
    ...
    try {
      ...
      // await createListingsAndReviews(client,[
      //   {
      //     name: "Infinite Views",
...
async function createListingsAndReviews(client,list){
  const result = await client.db("sample_airbnb").collection("listingsAndReviews").insert(list);

  console.log(`${result.insertedCount} new list(s) created with the following id(s):`);
  console.log(result.insertedIds);
}
